...
/** Booking items appear on /itinerary/car_rental; schedule items are day assignments only. */
export function isCarRentalBookingItem(item: ItineraryItem): boolean {
  ...
  if (details.isCarRentalBooking === true) return true;
  if (details.isCarRentalBooking === false) return false;
  return item.dayId == null;
}

export function isCarRentalScheduleItem(item: ItineraryItem): boolean {
  return item.category === "car_rental" && !isCarRentalBookingItem(item);
}
